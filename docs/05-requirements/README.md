# 05 - Requirements

Esta pasta concentra requisitos funcionais, nao funcionais, regras de negocio e criterios de aceite.

## Arquivos

- [Requisitos funcionais](functional-requirements.md)
- [Requisitos nao funcionais](non-functional-requirements.md)
- [Requisitos do Backoffice Fokus Cloud](backoffice.md)
- [Requisitos do Fokus Law](fokus-law.md)
- [Requisitos da gestao de processos](fokus-law-gestao-processual.md)
- [Requisitos da gestao de contatos](fokus-law-contatos.md)
- [Requisitos da gestao de expedicoes](fokus-law-expedicoes.md)
- [Requisitos da gestao de tarefas](fokus-law-tarefas-fluxos.md)
- [Requisitos do modulo identidade e acesso](identity-and-access.md)
- [Ciclo de vida da conta global](user-account-lifecycle.md)
- [Template de requisitos de modulo](module-requirements-template.md)
- [Regras de negocio](business-rules.md)

## Quando atualizar

Atualize esta pasta antes de implementar modulos, alterar fluxos principais ou mudar comportamento esperado.
